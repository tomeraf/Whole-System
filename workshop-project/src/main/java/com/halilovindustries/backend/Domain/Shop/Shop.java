    ...
    public HashMap<Integer,String> auctionMessages(){
        ...
        for (AuctionPurchase auction : pastAuctions) {
            if (!auction.isNotified()) {
                messages.put(auction.getBuyerId(), "You have won the auction for item: " + getItem(auction.getItemId()).getName() + ", in shop: " + name + ", with a bid of: " + auction.getHighestBid());
                for(int bidderId : auction.getBidders()) {
                    if (bidderId != auction.getBuyerId()) {
                        messages.put(bidderId, "You have lost the auction for item: " + getItem(auction.getItemId()).getName() + ", in shop: " + name + ", The winning bid was: " + auction.getHighestBid());
                    }
                }
                for(int ownerId: getOwnerIDs()) {
                    messages.put(ownerId, "The auction for item: " + getItem(auction.getItemId()).getName() + " in shop: " + name + " has ended. The winning bid was: " + auction.getHighestBid());
                }
