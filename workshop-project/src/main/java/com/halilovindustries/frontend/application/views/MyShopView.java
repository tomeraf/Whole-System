...
@Route(value = "shop/:shopID", layout = MainLayout.class)
...
public class MyShopView extends Composite<VerticalLayout> implements HasUrlParameter<Integer> {
    ...
    private final Button addItemButton = new Button("Add Item", VaadinIcon.PLUS.create());
    ...
    public MyShopView(MyShopPresenter presenter) {
        ...
        addItemButton.addClickListener(e -> openAddItemDialog());  // open your “add item” dialog
        ...
        itemsLayout.setJustifyContentMode(FlexComponent.JustifyContentMode.CENTER);
    ...
    private void openAddItemDialog() {
        ...
        dlg.add(new VerticalLayout(name, desc, price, qty, actions));
        dlg.open();
    }


    private Div createItemCard(ItemDTO item) {
        // 1) card wrapper
        Div card = new Div();
        card.setWidth("200px");
        card.getStyle()
            ...
            .set("cursor", "pointer")
            // make all cards occupy ~25% so you get 4 across (adjust as you like)
            .set("flex", "0 0 25%");

        // 4) title
        H4 title = new H4(item.getName());
        ...
        meta.setText("$" + item.getPrice() + " · ⭐" + item.getRating());
